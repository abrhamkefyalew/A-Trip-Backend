    ...
    public function store(StoreTripRequest $request)
    {
        ...
        $var = DB::transaction(function () use ($request) {
            ...
            if ($order->status !== Order::ORDER_STATUS_START) {
                return response()->json(['message' => 'this order is not STARTED. the order should have status: START before trip creation. order should be STARTED before a Trip can be created under it.'], 403); 
            }
            ...
            if ($order->contractDetail->with_fuel !== 1) {
                return response()->json(['message' => 'Trip can only be created for orders that require fuel. this Trip can Not be created, because its parent order does not require fuel'], 403);
            }
            ...
            if ($contract->terminated_date !== null) {
                ...
                return response()->json(['message' => 'Not Found - the server cannot find the requested resource. the Contract for the requested Order is Terminated.'], 404); 
            }
            ...
            //
            if (!$trip) {
                return response()->json(['message' => 'Trip Create Failed'], 422);
            }
