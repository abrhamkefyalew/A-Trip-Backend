...
use App\Http\Requests\Api\V1\DriverRequests\StartOrderRequest;
use App\Http\Requests\Api\V1\DriverRequests\AcceptOrderRequest;
use App\Http\Requests\Api\V1\DriverRequests\UpdateOrderRequest;
use App\Http\Requests\Api\V1\DriverRequests\CompleteOrderRequest;
use App\Http\Resources\Api\V1\OrderResources\OrderForDriverResource;
...
class OrderController extends Controller
{
     ...
     *      THIS MEANS = the logged in Driver can see a list of orders from orders table that have his driver_id
     * 
    ...
    public function index(Request $request)
    {
        ...
        $ordersData = $orders->with('vehicleName', 'vehicle', 'supplier', 'contractDetail', 'organization')
            ->latest()
        ...
        return OrderForDriverResource::collection($ordersData);
    }
    ...
    public function indexPending(Request $request)
    {
        ...
        // this 1 value is = since driver is trying to see orders. and driver should only see orders that have "contract_details.is_available = 1", which means the contract is NOT terminated or expired. which makes the contract_detail to have is_available 1
        ...
        // 
        $ordersData = $orders->whereHas('contractDetail', function (Builder $builder) use ($isAvailable) {
            $builder->where('contract_details.is_available', $isAvailable);
        });
        ...
        $ordersDataFinal = $ordersData->whereIn('vehicle_name_id', $vehicleNameIds)
            ...
            ->with('vehicleName', 'vehicle', 'supplier', 'contractDetail', 'organization')
            ->latest()
            ->paginate(FilteringService::getPaginate($request)); // get list of orders that fits this logged in driver


        return OrderForDriverResource::collection($ordersDataFinal);
    ...
    public function acceptOrder(AcceptOrderRequest $request, Order $order)
    {
        ...
        $var = DB::transaction(function () use ($request, $order) {
            ...
            //
            if ($order->contractDetail->with_driver == 0) {
            ...
            $supplier = Supplier::find($vehicle->supplier_id);
            ...
            if ($order->contractDetail->is_available !== 1) {       // also test if this condition is needed   // check abrham samson
            ...
            return OrderForDriverResource::make($updatedOrder->load('vehicleName', 'vehicle', 'supplier', 'contractDetail', 'organization'));
     ...
     * 
     * when the Driver (with the vehicle) Reaches the order maker of the organization at the Order start location
     */
     ...
     * 
     * when the Driver Finishes a trip or all trips, and the organization need for that vehicle is fulfilled by the Driver
     */
     ...
     * Update the specified resource in storage.
     * 
     * DRIVER Should NOT UPDATE ORDER 
     * 
     */
