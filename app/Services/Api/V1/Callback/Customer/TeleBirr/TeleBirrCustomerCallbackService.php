...
namespace App\Services\Api\V1\Callback\Customer\TeleBirr;
 ...
 * 
 */
class TeleBirrCustomerCallbackService
{
    private $invoiceUserId;

    public function __construct($invoiceReferenceWithPrefixFromTeleBirr)
    {
        // first lets strip out "ICI-" prefix from the invoice we got
        $this->invoiceUserId = substr($invoiceReferenceWithPrefixFromTeleBirr, 4); // Start from the 5th character onwards // THIS WORKS
        // $this->$invoiceUserIdInitialPayment = str_replace("ICI-", "", $invoiceReferenceWithPrefixFromTeleBirr);         // This Works ALSO
        // $this->$invoiceUserIdFinalPayment = str_replace("ICF-", "", $invoiceReferenceWithPrefixFromTeleBirr);           // This Works ALSO
    }
    ...
    {
        Log::alert('TeleBirr callback: ' . $message);
        abort($statusCode, 'TeleBirr callback: ' . $message);
    }
