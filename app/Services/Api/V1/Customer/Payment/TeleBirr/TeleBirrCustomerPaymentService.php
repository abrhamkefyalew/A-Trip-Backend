...
namespace App\Services\Api\V1\Customer\Payment\TeleBirr;
 ...
 * 
 */
class TeleBirrCustomerPaymentService
{    


    public function initiateInitialPaymentForVehicle($title, $amount)
    {
        // add prefix = "ICI0" : - prefix on the invoice code variable so that during call back later we could know that it is for INDIVIDUAL CUSTOMER INITIAL payment
        // $invoiceUserIdValWithPrefixInitial = config('constants.payment.customer_to_business.individual_customer_initial') . $this->createMerchantOrderId(); // add the ICI0 prefix to indicate the invoice code is for individual customer initial payment // we will use it later when the callback comes from the banks
        // $invoiceUserIdValWithPrefixInitial = $this->createMerchantOrderId();
        $invoiceUserIdValWithPrefixInitial = config('constants.payment.customer_to_business.individual_customer_initial') . (string) $title;

        $renderedViewForPayment = $this->createOrder($invoiceUserIdValWithPrefixInitial, $amount);

        return $renderedViewForPayment;
    }

    public function initiateFinalPaymentForVehicle($title, $amount)
    {
        // add prefix = "ICF0" : - prefix on the invoice code variable so that during call back later we could know that it is for INDIVIDUAL CUSTOMER FINAL payment
        // $invoiceUserIdValWithPrefixFinal = config('constants.payment.customer_to_business.individual_customer_final') . $this->createMerchantOrderId(); // add the ICF0 prefix to indicate the invoice code is for individual customer final payment // we will use it later when the callback comes from the banks
        // $invoiceUserIdValWithPrefixFinal = $this->createMerchantOrderId();
        $invoiceUserIdValWithPrefixFinal = config('constants.payment.customer_to_business.individual_customer_final') . (string) $title;

        $renderedViewForPayment = $this->createOrder($invoiceUserIdValWithPrefixFinal, $amount);
    ...
    private function requestCreateOrder($fabricToken, $title, $amount)
    {
        ...
        //
        Log::info("Telebirr (Customer Payment For Order): ALL of Request and Response Info FOR requestCreateOrder()" . json_encode($allRequestResponseInfoFORrequestCreateOrder_TO_BE_LOGGED));
